...
do_bar_chart <- function(data_path, out_path, threads, myparam) {
    ...
    ggsave(out_path[['plot1']], width=15, height=5.5)

    metrics = metrics %>% group_by(tool, genome) %>% slice_max(order_by=f1, n=1, with_ties=F)
    write_tsv(metrics, out_path[['table2']])
    ggplot(metrics, aes(y=f1, x=reorder(tool, f1), fill=tool)) + geom_bar(stat='identity') + facet_grid(~genome, scales='free')+theme_classic() + ylab('F1 score') + theme(axis.text.x = element_text(angle=90, size=12, hjust=1))
    ggsave(out_path[['plot2']], width=8, height=5.5)
}
