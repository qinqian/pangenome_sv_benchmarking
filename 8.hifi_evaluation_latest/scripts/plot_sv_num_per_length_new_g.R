...
do_bar_chart <- function(data_path, out_path, threads, myparam) {
    ...
    #pdf(out_path[['pdf']], width=9.6, height=3.2)
    #p=ggplot(parsed_res_thresholds, aes(x=reorder(file, value), y=value, fill=Size)) + geom_bar(position='stack', stat='identity') + ylab(expression("#germline SV")) + xlab("") + get_theme(size, 45)
    #print(p)
    #dev.off()
}
