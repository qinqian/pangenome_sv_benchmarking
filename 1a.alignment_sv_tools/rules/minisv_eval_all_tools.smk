...
for cutoff in [2,3,4,5,10]:
    ...
    rule:
        name: f"minisv_eval_single_mode{cutoff}_20kb"
        input:
            single = input_single_minisv_hg38,
            single2 = input_single_minisv_t2t,
            severus = "output/severus_{platform}/{cell_line}_{pair}_{assembly}",
            ...
            truth = input_truth,
        output:
            eval = f"output/minisv_eval_single{{pair}}/single_{{cell_line}}_{{platform}}_{{assembly}}_count{cutoff}_eval_len20kb.tsv"
        params:
            c = cutoff
        shell: 
            """
            if [[ {wildcards.assembly} == "chm13" ]]; then
                minisv.js eval -l 20e3 -c {params.c} -b ~/data/pangenome_sv_benchmarking/minisv/data/chm13v2.reg.bed {input.truth} {input.single2} {input.severus}/all_SVs/severus_all.vcf {input.svision} {input.sniffles_single} {input.sniffles_single_mosaic} > {output.eval}
            else
