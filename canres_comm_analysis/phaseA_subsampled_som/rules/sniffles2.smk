...
rule sniffles2_mosaic_downsample10:
    input:
        cram = expand("output/align/{{cell_line}}_{{platform}}/{pair}/{{assembly}}.cram", pair=["T", "BL"]),
        sz = expand("/hlilab/21data/collections/cancer-pair/{{cell_line}}{pair}.{{platform}}.fastq.sz", pair=["T", "BL"])
    output:
    ...
    shell:
        ...
        tumor_size=$(cat {input.sz[0]} | cut -f 2)
        normal_size=$(cat {input.sz[1]} | cut -f 2)
        ratio=$(echo \"scale=3; 1+(${{normal_size}}*0.10/${{tumor_size}})\" | bc -l)
        echo $ratio
        samtools/samtools merge --reference ../1a.alignment_sv_tools/{wildcards.assembly}.fa -o {output.cram} {input.cram[1]} <(samtools/samtools view --reference ../1a.alignment_sv_tools/{wildcards.assembly}.fa -us${{ratio}} {input.cram[0]})
        samtools/samtools index {output.cram} {output.crai}
        """
...
rule sniffles2_mosaic_downsample10_call:
    input:
        cram = "output/align/{cell_line}_{platform}/{assembly}_mixdown10.cram",
        crai = "output/align/{cell_line}_{platform}/{assembly}_mixdown10.crai"
    output:
        vcf = "output/sniffles_mosaic/{cell_line}_{platform}/{assembly}_mixdown10_mosaic.vcf.gz",
        tbi = "output/sniffles_mosaic/{cell_line}_{platform}/{assembly}_mixdown10_mosaic.vcf.gz.tbi"
    params:
        mosaic= "--mosaic"
    conda: "sniffles2"
    threads: 4
...
rule sniffles2_snf_downsample10:
    ...
    input:
        ...
        crai = "output/align/{cell_line}_{platform}/{assembly}_mixdown10.crai"
    output:
        snf = "output/sniffles/{cell_line}_{platform}/{assembly}_mixdown10.snf"
    resources:
        runtime="15h",
        mem_mb_per_cpu=2000,
        tmpdir="local_tmp/"
...
rule sniffles2_tumor_normal_pair_downsample10:
    input:
        mix = "output/sniffles/{cell_line}_{platform}/{assembly}_mixdown10.snf",
        snf = expand("output/sniffles/{{cell_line}}_{{platform}}/{pair}/{{assembly}}.snf", pair=["T", "BL"])
    output:
        vcf = "output/sniffles/{cell_line}_{platform}/{assembly}_multi_mixdown10.vcf.gz",
        tbi = "output/sniffles/{cell_line}_{platform}/{assembly}_multi_mixdown10.vcf.gz.tbi"
    conda: "sniffles2"
    threads: 1
    resources:
        runtime="15h",
    ...
    shell:
        """
        sniffles --reference ../1a.alignment_sv_tools/{wildcards.assembly}.fa --tandem-repeats ../1a.alignment_sv_tools/{wildcards.assembly}_vntrs.bed --input {input.mix} {input.snf[1]} --output-rnames --vcf {output.vcf}
        """


rule sniffles2_snf_downsample25:
    threads: 12
    conda: "sniffles2"
    input:
        cram = "output/align/{cell_line}_{platform}/{assembly}_mixdown.cram",
        crai = "output/align/{cell_line}_{platform}/{assembly}_mixdown.crai"
    output:
        snf = "output/sniffles/{cell_line}_{platform}/{assembly}_mixdown.snf"
    resources:
        runtime="15h",
        mem_mb_per_cpu=2000,
        tmpdir="local_tmp/"
...
rule sniffles2_tumor_normal_pair_downsample25:
    input:
        mix = "output/sniffles/{cell_line}_{platform}/{assembly}_mixdown.snf",
        snf = expand("output/sniffles/{{cell_line}}_{{platform}}/{pair}/{{assembly}}.snf", pair=["T", "BL"])
    output:
        vcf = "output/sniffles/{cell_line}_{platform}/{assembly}_multi_mixdown25.vcf.gz",
        tbi = "output/sniffles/{cell_line}_{platform}/{assembly}_multi_mixdown25.vcf.gz.tbi"
    conda: "sniffles2"
    threads: 1
    resources:
        runtime="15h",
