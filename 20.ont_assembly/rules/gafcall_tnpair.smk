rule minisv_tnpair_tgs_extract_tumor:
    ...
    input:
        ...
        asm = expand("output/align/{{cell_line}}_{pair}_{{platform}}_s.paf.gz", pair=['T', 'BL'])
    output:
...
rule minisv_tnpair_somatic_call:
    ...
    resources:
        ...
        run_time="12h"
    output:
        msv="output/minisv_pair/{cell_line}_{platform}_pair_hg38l_l+t+g+s_c2s1.msv.gz"
    shell:
        """
        cat {input.t} <(zcat {input.n}) | sort -k1,1 -k2,2n -S4g \
          | minisv.js merge -c 2 -s 1 - | grep TUMOR | grep -v NORMAL | gzip > {output.msv}
        """
...
rule gafcall_merge_filter_tnpair_hg38:
    ...
    output:
        msv = expand("output/minisv_pair/{{cell_line}}_{{platform}}_pair_hg38l_l+t+g+s_{cnt}.msv.gz", cnt=['c3s1', 'c4s1', 'c5s1'])
    params:
        c = [3, 4, 5],
        s = [1, 1, 1]
    resources:
...
####     output:
####         msv = "output/minisv/{cell_line}_{pair}_{platform}_grch38l_t+g_merge_c2s1.msv.gz"
####     resources:
####         mem_mb=24000, 
####         run_time="8h"
####     shell:
####         "k8 --max-old-space={resources.mem_mb} ~/data/pangenome_sv_benchmarking/minisv/minisv.js isec {input.rsv[1]} {input.rsv[0]} |k8 --max-old-space={resources.mem_mb} ~/data/pangenome_sv_benchmarking/minisv/minisv.js isec - {input.graph[0]} | sort -k1,1 -k2,2n -S4g | ~/data/pangenome_sv_benchmarking/minisv/minisv.js merge -c 2 -s 1 - | gzip > {output.msv}"
#### 
#### 
...
####     output:
####         msv = expand("output/minisv/{{cell_line}}_{{pair}}_{{platform}}_grch38l_t+g_merge_{cnt}.msv.gz", cnt=['c3s1', 'c4s1', 'c5s1'])
####     params:
####         c = [3, 4, 5],
####         s = [1, 1, 1]
...
rule minisv_tnpair_somatic_call_tg:
    ...
    resources:
        ...
        run_time="12h"
    output:
        msv="output/minisv_pair/{cell_line}_{platform}_pair_hg38l_l+tg_c2s1.msv.gz"
    shell:
        """
...
use rule gafcall_merge_filter_tnpair_hg38 as tnpair_gafcall_merge_join_tg_filter with:
    input:
        msv = rules.minisv_tnpair_somatic_call_tg.output.msv,
    output:
        msv = expand("output/minisv_pair/{{cell_line}}_{{platform}}_pair_hg38l_l+tg_{cnt}.msv.gz", cnt=['c3s1', 'c4s1', 'c5s1'])
    params:
        c = [3, 4, 5],
        s = [1, 1, 1]
