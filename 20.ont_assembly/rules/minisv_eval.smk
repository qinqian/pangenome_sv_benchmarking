...
for cutoff in [2,3,4,5,6,7]:
    rule:
        name: f"latest_colo_ont12_minisv_somatic_eval_puretumor_count{cutoff}"
        input:
            severus = "../1a.alignment_sv_tools/output/severus_latest/{cell_line}_{platform}/{assembly}_cutoff2_read_ids/somatic_SVs/severus_somatic.vcf",
            severus_read_ids = "../1a.alignment_sv_tools/output/severus_latest/{cell_line}_{platform}/{assembly}_cutoff2_read_ids/read_ids.csv",
            
            savana = "../1a.alignment_sv_tools/output/savana13/{cell_line}_{platform}/{assembly}/{assembly}_T_tag.classified.somatic.vcf",
            savana_read_ids = "../1a.alignment_sv_tools/output/savana13/{cell_line}_{platform}/{assembly}/{assembly}_T_tag.sv_breakpoints_read_support.tsv",

            nanomonsv = "../1a.alignment_sv_tools/output/nanomonsv_latest/{cell_line}_{platform}/{assembly}_tnpair.vcf",
            nanomonsv_read_ids = "../1a.alignment_sv_tools/output/nanomonsv_latest/{cell_line}_{platform}/T/grch38_parse.nanomonsv.supporting_read.txt",

            snf2 = "../1a.alignment_sv_tools/output/sniffles_latest/{cell_line}_{platform}/{assembly}_multi.vcf.gz",
            minisv_ltg = "output/minisv_pair/{cell_line}_{platform}_pair_hg38l_l+tg_c2s1.msv.gz",
            ...
            truth = input_truth
        output:
            severus_filtered_asm     = f"output/latest_minisv_puretumor_somatic_asm/severus_{{cell_line}}_{{platform}}_{{assembly}}_somatic_generation{cutoff}_filterasm.vcf",

            msv_somatic_filtered_asm = f"output/latest_minisv_puretumor_somatic_asm/msv_ltgs_{{cell_line}}_{{platform}}_{{assembly}}_somatic_generation{cutoff}_filterasm.vcf",

            savana_somatic_filtered_asm = f"output/latest_minisv_puretumor_somatic_asm/savana_{{cell_line}}_{{platform}}_{{assembly}}_somatic_generation{cutoff}_filterasm.vcf",
            nanomonsv_somatic_filtered_asm = f"output/latest_minisv_puretumor_somatic_asm/nanomonsv_{{cell_line}}_{{platform}}_{{assembly}}_somatic_generation{cutoff}_filterasm.vcf",

            snf_somatic              = f"output/latest_minisv_puretumor_somatic_asm/snf_{{cell_line}}_{{platform}}_{{assembly}}_somatic_generation{cutoff}.vcf",
            snf_somatic_filtered_asm = f"output/latest_minisv_puretumor_somatic_asm/snf_{{cell_line}}_{{platform}}_{{assembly}}_somatic_generation{cutoff}_filterasm.vcf",

            out_snf_filterstat       = f"output/latest_minisv_puretumor_somatic_asm/snf_{{cell_line}}_{{platform}}_{{assembly}}_somatic_generation{cutoff}_filterasm.stat",
            out_msv_filterstat       = f"output/latest_minisv_puretumor_somatic_asm/msv_ltgs_{{cell_line}}_{{platform}}_{{assembly}}_somatic_generation{cutoff}_filterasm.stat",
            out_severus_filterstat   = f"output/latest_minisv_puretumor_somatic_asm/severus_{{cell_line}}_{{platform}}_{{assembly}}_somatic_generation{cutoff}_filterasm.stat",
            out_savana_filterstat   = f"output/latest_minisv_puretumor_somatic_asm/savana_{{cell_line}}_{{platform}}_{{assembly}}_somatic_generation{cutoff}_filterasm.stat",
            out_nanomonsv_filterstat   = f"output/latest_minisv_puretumor_somatic_asm/nanomonsv_{{cell_line}}_{{platform}}_{{assembly}}_somatic_generation{cutoff}_filterasm.stat",

            eval = f"output/latest_minisv_puretumor_somatic_asm/{{cell_line}}_{{platform}}_{{assembly}}_count{cutoff}_eval.tsv"
        conda: 'msvpy'
        params:
            c = cutoff
        shell: 
