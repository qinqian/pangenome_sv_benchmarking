...
rule sniffles2_mosaic_downsample:
    input:
        cram = expand("output/align/{{cell_line}}/{pair}/{{assembly}}.cram", pair=["T", "BL"]),
        sz = expand("{{cell_line}}_{pair}.fastq.gz.sz", pair=["T", "BL"])
    output:
...
rule query_downsample_paf:
    input:
        cram = "output/align/{cell_line}/{assembly}_mixdown.cram",
        crai = "output/align/{cell_line}/{assembly}_mixdown.crai",
        pafs = expand("output/align/{{cell_line}}_{pair}_{{assembly}}l.paf.gz", pair=['T', 'BL'])
    output:
...
rule query_downsample_gaf:
    input:
        cram = "output/align/{cell_line}/{assembly}_mixdown.cram",
        crai = "output/align/{cell_line}/{assembly}_mixdown.crai",
        pafs = expand("output/align/{{cell_line}}_{pair}_{{assembly}}g.paf.gz", pair=['T', 'BL'])
    output:
