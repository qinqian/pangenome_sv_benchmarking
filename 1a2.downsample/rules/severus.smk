...
pwd = '/hlilab/alvin/pangenome_sv_benchmarking/1a2.downsample'
celllines = ['COLO829']
assembly = ['grch38', 'chm13']

idx_files = expand(os.path.join(pwd, "output/align/{cell_line}/{pair}/{assembly}_{pair}_tag.cram.crai"), cell_line=celllines, pair=["T", "BL"], assembly=assembly)
files     = expand(os.path.join(pwd, "output/align/{cell_line}/{pair}/{assembly}_{pair}_tag.cram"), cell_line=celllines, pair=["T", "BL"], assembly=assembly)
severus_files     = expand("output/severus/{cell_line}/{assembly}", cell_line=celllines, assembly=assembly)
...
rule haplotag:
    input:
         phased_vcf = "output/clair3/{cell_line}/{pair}/{assembly}",
         cram = "output/align/{cell_line}/{pair}/{assembly}.cram",
         crai = "output/align/{cell_line}/{pair}/{assembly}.cram.crai"
    output:
         haplotag_cram = os.path.join(pwd, "output/align/{cell_line}/{pair}/{assembly}_tag.cram"),
         haplotag_crai = os.path.join(pwd, "output/align/{cell_line}/{pair}/{assembly}_tag.cram.crai")
    resources:
         mem_mb=64000
    conda: "clair3"
    threads: 8
...
rule severus_linktag:
    input:
        crams = os.path.join(pwd,"output/align/{cell_line}/{pair}/{assembly}_tag.cram"),
        crais = os.path.join(pwd,"output/align/{cell_line}/{pair}/{assembly}_tag.cram.crai")
    output:
        crams = os.path.join(pwd, "output/align/{cell_line}/{pair}/{assembly}_{pair}_tag.cram"),
        crais = os.path.join(pwd, "output/align/{cell_line}/{pair}/{assembly}_{pair}_tag.cram.crai")
    shell:
        """
...
rule severus_tumor_normal_pair:
    input:
        crams = expand(os.path.join(pwd, "output/align/{{cell_line}}/{pair}/{{assembly}}_{pair}_tag.cram"), pair=["T", "BL"]),
        crais = expand(os.path.join(pwd, "output/align/{{cell_line}}/{pair}/{{assembly}}_{pair}_tag.cram.crai"), pair=["T", "BL"]),
        phased_vcf = expand("output/clair3/{{cell_line}}/{pair}/{{assembly}}", pair=["T", "BL"])
    output:
