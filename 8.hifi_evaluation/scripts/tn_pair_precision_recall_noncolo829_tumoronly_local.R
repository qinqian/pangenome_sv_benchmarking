...
load_metrics <- function(data_path) {
    ...
    metrics$tool = gsub("severus_data2", "", metrics$file)
    metrics$tool = gsub("severus_data", "", metrics$tool)
    metrics$tool = str_extract(metrics$tool, "(gafcall|minisv|nanomonsv|savana|severus|sniffles_mosaic|sniffles|svision|cutesv)", group=1)
#l_l+t+g+s
    metrics$tool = gsub('gafcall', 'minisv', metrics$tool)
    metrics$param = str_extract(metrics$file, "(l\\+g|l\\+tg|l\\+g|l_l\\+t\\+g\\+s|l_l\\+t\\+g\\+s|l_l\\+x|g_g\\+x|l_l\\+g|l_l\\+t|l\\+tgs|l\\+gs|l\\+ts|l\\+tg)(_c2s0|\\.pair-c2s0)", group=1)
    metrics$param[is.na(metrics$param)] = ""
    metrics$tool = paste0(metrics$tool, metrics$param)
    metrics$tool = gsub('minisv', 'msv:tg', metrics$tool)
    metrics
}
...
do_bar_chart <- function(input, out_path, threads, myparam) {
    ...
    metrics.ont = load_metrics(data_path)
