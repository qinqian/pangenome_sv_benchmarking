rule minisv_tnpair_tgs_extract_tumor_chm13:
    ...
    output:
        t_rsv = "output/minisv_pair/{cell_line}_tumor_chm13l+tgs.rsv"
    shell:
        """
...
rule minisv_tnpair_somatic_call_chm13:
    ...
    resources:
        ...
        run_time="12h"
    output:
        msv="output/minisv_pair/{cell_line}_pair_chm13l_l+t+g+s_c2s0.msv.gz"
    shell:
        """
...
rule gafcall_merge_filter_tnpair_chm13:
    ...
    output:
        msv = expand("output/minisv_pair/{{cell_line}}_pair_chm13l_l+t+g+s_{cnt}.msv.gz", cnt=['c3s0', 'c4s0', 'c5s0'])
    params:
        c = [3, 4, 5],
        s = [0, 0, 0]
    resources:
...
rule minisv_tnpair_tgs_extract_tumor:
    ...
    input:
        ...
        asm = expand("{{cell_line}}{pair}_asm.paf.gz", pair=['T', 'BL'])
    output:
        t_rsv = "output/minisv_pair/{cell_line}_tumor_hg38l+tgs.rsv"
    shell:
        """
...
rule minisv_tnpair_somatic_call:
    ...
    resources:
        ...
        run_time="12h"
    output:
        msv="output/minisv_pair/{cell_line}_pair_hg38l_l+t+g+s_c2s0.msv.gz"
    shell:
        """
        cat {input.t} <(zcat {input.n}) | sort -k1,1 -k2,2n -S4g \
...
rule gafcall_merge_filter_tnpair_hg38:
    ...
    output:
        msv = expand("output/minisv_pair/{{cell_line}}_pair_hg38l_l+t+g+s_{cnt}.msv.gz", cnt=['c3s0', 'c4s0', 'c5s0'])
    params:
        c = [3, 4, 5],
        s = [0, 0, 0]
    resources:
...
rule gafcall_extract_asm:
    ...
    output:
        rsv = "output/minisv/{cell_line}_{pair}_self.rsv.gz"
    shell:
        """
...
rule gafcall_extract:
    ...
    output:
        rsv = "output/minisv/{cell_line}_{pair}_{assembly}l.def.rsv.gz"
    shell:
        """
        if [[ {wildcards.pair} == "T" ]]; then
...
rule gafcall_extract_graph:
    ...
    output:
        rsv = "output/minisv/{cell_line}_{pair}_{assembly}g.def.rsv.gz"
    shell:
        """
...
rule gafcall_merge_alone:
    ...
    output:
        msv = "output/minisv/{cell_line}_{pair}_{assembly}l_l+x_merge_c2s0.msv.gz"
    threads: 1
    resources:
        mem_mb=4000, 
        run_time="1h"
...
rule gafcall_merge_filter:
    ...
    output:
        msv = expand("output/minisv/{{cell_line}}_{{pair}}_{{assembly}}l_l+x_merge_{cnt}.msv.gz", cnt=['c3s0', 'c4s0', 'c5s0'])
    params:
        c = [3, 4, 5],
        s = [0, 0, 0]
    resources:
...
rule gafcall_merge_join_graph:
    ...
    output:
        msv = "output/minisv/{cell_line}_{pair}_{assembly}l_l+g_merge_c2s0.msv.gz"
    resources:
        mem_mb=24000, 
        run_time="8h"
    shell:
...
use rule gafcall_merge_filter as gafcall_merge_join_graph_filter with:
    ...
    output:
        msv = expand("output/minisv/{{cell_line}}_{{pair}}_{{assembly}}l_l+g_merge_{cnt}.msv.gz", cnt=['c3s0', 'c4s0', 'c5s0'])
    params:
        c = [3, 4, 5],
        s = [0, 0, 0]
...
rule gafcall_merge_join_tg:
    input:
        rsv = expand("output/minisv/{{cell_line}}_{{pair}}_{assembly}l.def.rsv.gz", assembly=['chm13', 'grch38']),
        graph = expand("output/minisv/{{cell_line}}_{{pair}}_{assembly}g.def.rsv.gz", assembly=['chm13', 'grch38'])
    output:
        msv = "output/minisv/{cell_line}_{pair}_grch38l_t+g_merge_c2s0.msv.gz"
    resources:
        mem_mb=24000, 
        run_time="8h"
    shell:
...
use rule gafcall_merge_filter as gafcall_merge_join_tg_filter with:
    ...
    output:
        msv = expand("output/minisv/{{cell_line}}_{{pair}}_grch38l_t+g_merge_{cnt}.msv.gz", cnt=['c3s0', 'c4s0', 'c5s0'])
    params:
        c = [3, 4, 5],
        s = [0, 0, 0]
...
rule minisv_tnpair_somatic_call_tg:
    input:
        rsv = expand("output/minisv/{{cell_line}}_T_{assembly}l.def.rsv.gz", assembly=['chm13', 'grch38']),
        graph = expand("output/minisv/{{cell_line}}_T_{assembly}g.def.rsv.gz", assembly=['chm13', 'grch38']),
        n = rules.minisv_tnpair_tgs_extract_normal.output.n_rsv
    resources:
        mem_mb=24000, 
        run_time="12h"
    output:
        msv="output/minisv_pair/{cell_line}_pair_hg38l_l+tg_c2s0.msv.gz"
    shell:
        """
...
use rule gafcall_merge_filter as tnpair_gafcall_merge_join_tg_filter with:
    ...
    output:
        msv = expand("output/minisv_pair/{{cell_line}}_pair_hg38l_l+tg_{cnt}.msv.gz", cnt=['c3s0', 'c4s0', 'c5s0'])
    params:
        c = [3, 4, 5],
        s = [0, 0, 0]
...
rule minisv_tnpair_somatic_call_g:
    ...
    resources:
        ...
        run_time="12h"
    output:
        msv="output/minisv_pair/{cell_line}_pair_hg38l_l+g_c2s0.msv.gz"
    shell:
        """
...
use rule gafcall_merge_filter as tnpair_gafcall_merge_join_g_filter with:
    input:
        msv = rules.minisv_tnpair_somatic_call_tg.output.msv,
    output:
        msv = expand("output/minisv_pair/{{cell_line}}_pair_hg38l_l+g_{cnt}.msv.gz", cnt=['c3s0', 'c4s0', 'c5s0'])
    params:
        c = [3, 4, 5],
        s = [0, 0, 0]
...
rule minisv_tnpair_somatic_call_t:
    ...
    resources:
        ...
        run_time="12h"
    output:
        msv="output/minisv_pair/{cell_line}_pair_hg38l_l+t_c2s0.msv.gz"
    shell:
        """
...
use rule gafcall_merge_filter as tnpair_gafcall_merge_join_t_filter with:
    input:
        msv = rules.minisv_tnpair_somatic_call_tg.output.msv,
    output:
        msv = expand("output/minisv_pair/{{cell_line}}_pair_hg38l_l+t_{cnt}.msv.gz", cnt=['c3s0', 'c4s0', 'c5s0'])
    params:
        c = [3, 4, 5],
        s = [0, 0, 0]
...
rule minisv_tnpair_somatic_call_lx:
    input:
        rsv = expand("output/minisv/{{cell_line}}_{pair}_{{assembly}}l.def.rsv.gz", pair=['T', 'BL'])
    resources:
        mem_mb=24000, 
        run_time="12h"
    output:
        msv="output/minisv_pair/{cell_line}_pair_{assembly}l_l+x_c2s0.msv.gz"
    shell:
        """
...
use rule gafcall_merge_filter as tnpair_gafcall_merge_join_lx_filter with:
    ...
    output:
        msv = expand("output/minisv_pair/{{cell_line}}_pair_{{assembly}}l_l+x_{cnt}.msv.gz", cnt=['c3s0', 'c4s0', 'c5s0'])
    params:
        c = [3, 4, 5],
        s = [0, 0, 0]
...
rule minisv_tnpair_somatic_call_gx:
    input:
        rsv = expand("output/minisv/{{cell_line}}_{pair}_{{assembly}}g.def.rsv.gz", pair=['T', 'BL'])
    resources:
        mem_mb=24000, 
        run_time="12h"
    output:
        msv="output/minisv_pair/{cell_line}_pair_{assembly}g_g+x_c2s0.msv.gz"
    shell:
        """
        zcat {input.rsv[0]} {input.rsv[1]} | sort -k1,1 -k2,2n -S4g \
          | minisv.js merge -c 2 -s 0 - | grep TUMOR | grep -v NORMAL | gzip > {output.msv}
...
use rule gafcall_merge_filter as tnpair_gafcall_merge_join_gx_filter with:
    ...
    output:
        msv = expand("output/minisv_pair/{{cell_line}}_pair_{{assembly}}g_g+x_{cnt}.msv.gz", cnt=['c3s0', 'c4s0', 'c5s0'])
    params:
        c = [3, 4, 5],
        s = [0, 0, 0]
...
rule gafcall_merge_join_s:
    input:
        rsv = "output/minisv/{cell_line}_{pair}_{assembly}l.def.rsv.gz",
        asm = "output/minisv/{cell_line}_{pair}_self.rsv.gz"
    output:
        msv = "output/minisv/{cell_line}_{pair}_{assembly}l_l+s_merge_c2s0.msv.gz"
    resources:
        mem_mb=24000, 
        run_time="8h"
    shell:
...
use rule gafcall_merge_filter as gafcall_merge_join_s_filter with:
    ...
    output:
        msv = expand("output/minisv/{{cell_line}}_{{pair}}_{{assembly}}l_l+s_merge_{cnt}.msv.gz", cnt=['c3s0', 'c4s0', 'c5s0'])
    params:
        c = [3, 4, 5],
        s = [0, 0, 0]
...
rule gafcall_merge_join_ts:
    ...
    resources:
        ...
        run_time="8h"
    output:
        msv = "output/minisv/{cell_line}_{pair}_grch38l_t+s_merge_c2s0.msv.gz"
    shell:
...
use rule gafcall_merge_filter as gafcall_merge_join_ts_filter with:
    ...
    output:
        msv = expand("output/minisv/{{cell_line}}_{{pair}}_grch38l_t+s_merge_{cnt}.msv.gz", cnt=['c3s0', 'c4s0', 'c5s0'])
    params:
        c = [3, 4, 5],
        s = [0, 0, 0]
...
rule gafcall_merge_join_lgs:
    ...
    resources:
        ...
        run_time="8h"
    output:
        msv = "output/minisv/{cell_line}_{pair}_{assembly}l_l+g+s_merge_c2s0.msv.gz"
    shell:
...
use rule gafcall_merge_filter as gafcall_merge_join_ts_filter_lgs with:
    ...
    output:
        msv = expand("output/minisv/{{cell_line}}_{{pair}}_{{assembly}}l_l+g+s_merge_{cnt}.msv.gz", cnt=['c3s0', 'c4s0', 'c5s0'])
    params:
        c = [3, 4, 5],
        s = [0, 0, 0]
...
rule gafcall_merge_join_tgs:
    input:
        ...
        asm = "output/minisv/{cell_line}_{pair}_self.rsv.gz"
    output:
        msv = "output/minisv/{cell_line}_{pair}_grch38l_t+g+s_merge_c2s0.msv.gz"
    resources:
        mem_mb=24000, 
        run_time="8h"
    shell:
...
use rule gafcall_merge_filter as gafcall_merge_join_tgs_filter with:
    ...
    output:
        msv = expand("output/minisv/{{cell_line}}_{{pair}}_grch38l_t+g+s_merge_{cnt}.msv.gz", cnt=['c3s0', 'c4s0', 'c5s0'])
    params:
        c = [3, 4, 5],
        s = [0, 0, 0]
...
rule minisv_view_length_tn:
    input:
        pair=expand("output/minisv_pair/{{cell_line}}_pair_{comb}.msv", comb=['chm13l_l+t+g+s', 'hg38l+tgs']),
    output:
        tn_eval_length = "output/minisv_view/tn_{cell_line}_eval_len.tsv",
    shell:
        """
...
rule minisv_view_length_single_count_grch38:
   input:
        single=expand("output/minisv/{{cell_line}}_{pair}_{assembly}l_{{comb}}_merge_{{cnt}}.msv.gz", assembly=['grch38'], pair=['T', 'BL'])
   output:
        single_eval_length = "output/minisv_view/single_grch38_{cell_line}_{comb}_merge_{cnt}_eval_len.tsv"
   shell:
        """
...
use rule minisv_view_length_single_count_grch38 as minisv_view_length_single_count_chm13 with:
   input:
        single=expand("output/minisv/{{cell_line}}_{pair}_{assembly}l_{{comb}}_merge_{{cnt}}.msv.gz", assembly=['chm13', 'grch38'], pair=['T', 'BL'])
   output:
        single_eval_length = "output/minisv_view/single_chm13_{cell_line}_{comb}_merge_{cnt}_eval_len.tsv"


rule minisv_view_length_single_combined:
    input: 
        single = expand("output/minisv_view/single_grch38_{{cell_line}}_{comb}_merge_{cnt}_eval_len.tsv", comb=['t+g+s', 't+s', 'l+s', 't+g', 'l+g', 'l+x'], cnt=['c2s0', 'c3s0', 'c4s0', 'c5s0']) + expand("output/minisv_view/single_{assembly}_{{cell_line}}_{comb}_merge_{cnt}_eval_len.tsv", comb=['l+x', 'l+g', 'l+s'], cnt=['c2s0', 'c3s0', 'c4s0', 'c5s0'], assembly=['chm13', 'grch38'])
    output:
        single_eval_length = "output/minisv_view/single_{cell_line}_eval_len.tsv"
    shell:
