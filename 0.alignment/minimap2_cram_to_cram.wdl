...
task minimapTask {
    ...
    command <<<
        samtools fasta -@ ~{cpu} --reference ~{input_cram_reference} ~{input_cram} | minimap2 -ax ~{mode} -t ~{cpu} ~{assembly} - | samtools sort --write-index -l 1 -@ ~{cpu} --output-fmt CRAM --reference ~{assembly} > ~{sample_id}.cram
    >>>
