rule sniffles2:
    ...
    output:
        vcf = "output/sniffles/{cell_line}/{pair}/{assembly}.vcf.gz",
        tbi = "output/sniffles/{cell_line}/{pair}/{assembly}.vcf.gz.tbi"
    shell:
        """
        sniffles --reference ../1a.alignment_sv_tools/{wildcards.assembly}.fa --tandem-repeats ../1a.alignment_sv_tools/{wildcards.assembly}_vntrs.bed --threads {threads} -i {input.cram} -v {output.vcf} --output-rnames --sample-id {wildcards.cell_line}_{wildcards.pair} {params.mosaic}
        """
...
rule sniffles2_tumor_normal_pair:
    input:
        snf = expand("output/sniffles/{{cell_line}}/{pair}/{{assembly}}.snf", pair=["tumor", "normal"])
    output:
...
rule sniffles2_mosaic_downsample:
    input:
        cram = expand("output/align/{{cell_line}}/{pair}/{{assembly}}.cram", pair=["tumor", "normal"]),
        sz = expand("{{cell_line}}_{pair}.fastq.gz.sz", pair=["tumor", "normal"])
    output:
...
rule sniffles2_mosaic_downsample_call:
    ...
    shell:
        """
        sniffles --threads {threads} --reference {wildcards.assembly}.fa --tandem-repeats {wildcards.assembly}_vntrs.bed -i {input.cram} -v {output.vcf} --output-rnames --sample-id {wildcards.cell_line} {params.mosaic}
        """
...
rule query_downsample_paf:
    input:
        cram = "output/align/{cell_line}/{assembly}_mixdown.cram",
        crai = "output/align/{cell_line}/{assembly}_mixdown.crai",
        pafs = expand("output/align/{{cell_line}}_{pair}_{{assembly}}l.paf.gz", pair=['tumor', 'normal'])
    output:
...
rule query_downsample_gaf:
    input:
        cram = "output/align/{cell_line}/{assembly}_mixdown.cram",
        crai = "output/align/{cell_line}/{assembly}_mixdown.crai",
        pafs = expand("output/align/{{cell_line}}_{pair}_{{assembly}}g.paf.gz", pair=['tumor', 'normal'])
    output:
