...
do_bar_chart <- function(input, out_path, threads, myparam) {
    ...
    for (file in c(data_path1)) {
        ...
        data_list1[[file]] = data_list1[[file]] %>% mutate(cell_line = gsub("_new_interface", "", basename(dirname(file))))%>% mutate(tool=gsub("_l\\+s_3_filtered\\.stat", "", basename(file)))
    }
    data_list2 = list()
    for (file in c(data_path2)) {
        data_list2[[file]] = read_tsv(file, col_names=T)
        data_list2[[file]] = data_list2[[file]] %>% mutate(cell_line = gsub("_new_interface", "", basename(dirname(file))))%>% mutate(tool=gsub("_l\\+s_3_filtered\\.stat", "", basename(file)))
    }
    data_list3 = list()
    for (file in c(data_path3)) {
        data_list3[[file]] = read_tsv(file, col_names=T)
        data_list3[[file]] = data_list3[[file]] %>% mutate(cell_line = gsub("_new_interface", "", basename(dirname(file)))) %>% mutate(tool=gsub("_l\\+s_3_filtered\\.stat", "", basename(file)))
    }
    data_list4 = list()
    for (file in c(data_path4)) {
        data_list4[[file]] = read_tsv(file, col_names=T)
        data_list4[[file]] = data_list4[[file]] %>% mutate(cell_line = gsub("_new_interface", "", basename(dirname(file))))%>% mutate(tool=gsub("_l\\+s_3_filtered\\.stat", "", basename(file)))
    }
