...
rule all:
    input:
        "output/single/batch12/colo829_ont_clean.tsv",
        "output/single/batch12/colo829_ont_clean.png",


rule colo829_evaluation:
    input:
        expand(os.path.join(config['samples']['tumor_normal_pair']['batch1']['dir'], 'output/minisv_eval_singleT', 'single_{cell_line}_{assembly}_count{count}_eval.tsv'), cell_line=config['samples']['tumor_normal_pair']['batch1']['cell_line1'], count=[2,3,4,5,10], assembly=['chm13', 'grch38'])+expand(os.path.join(config['samples']['tumor_normal_pair']['batch2']['dir2'], 'output/minisv_eval_singleT', 'single_{cell_line}_ont1_{assembly}_count{count}_eval.tsv'), cell_line=config['samples']['tumor_normal_pair']['batch2']['cell_line2'], count=[2,3,4,5,10], assembly=['chm13', 'grch38'])+expand(os.path.join(config['samples']['tumor_normal_pair']['batch2']['dir2'], 'output/minisv_eval_singleT', 'single_{cell_line}_ont2_{assembly}_count{count}_eval.tsv'), cell_line=config['samples']['tumor_normal_pair']['batch2']['cell_line2'], count=[2,3,4,5,10], assembly=['chm13', 'grch38'])
    conda: "plot"
    output:
        table1 = "output/single/batch12/colo829_ont_clean.tsv",
        table2 = "output/single/batch12/colo829_ont_clean_max.tsv",
        plot1 = "output/single/batch12/colo829_ont_clean.png",
        plot2 = "output/single/batch12/colo829_ont_clean_max.png",
    script:
        "scripts/single_precision_recall.R"
